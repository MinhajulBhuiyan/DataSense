...
\subsection{When -- Project Timeline}
...
\begin{keybox}[Git Repository Status]
\begin{itemize}
    \item \textbf{Repository}: \href{https://github.com/MinhajulBhuiyan/DataSense.git}{github.com/MinhajulBhuiyan/DataSense}
    \item \textbf{Clone}: \texttt{git clone https://github.com/MinhajulBhuiyan/DataSense.git}
    \item \textbf{Default branch}: \texttt{main}
    \item \textbf{Recommended workflow}: Feature branches, pull requests, and code review
    \item \textbf{License}: See the repository's LICENSE file for terms
\end{itemize}
\end{keybox}

\subsection{Contribution Workflow}
Follow a simple GitHub workflow to contribute:
\begin{itemize}
    \item Create a feature branch: \texttt{git checkout -b feature/your-description}
    \item Commit changes with meaningful messages and reference issues when relevant
    \item Push branch and open a Pull Request on GitHub targeting \texttt{main}
    \item Request reviews, address feedback, and merge after approvals and checks pass
\end{itemize}

\subsection{Clone and Run (Quick Start)}
Clone the repository and run the backend and frontend locally:
\begin{lstlisting}[language=bash]
# Clone
git clone https://github.com/MinhajulBhuiyan/DataSense.git
cd DataSense
...
\begin{itemize}
    \item Keep dependency files (\texttt{requirements.txt}, \texttt{package.json}) up to date
    \item Run unit and integration tests before opening PRs
    \item Use issue templates and descriptive PR titles to help reviewers
\end{itemize}
